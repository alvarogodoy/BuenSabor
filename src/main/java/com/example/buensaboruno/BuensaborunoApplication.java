...
public class BuensaborunoApplication {
                           ...
                           FacturaRepository facturaRepository)
    {
        return args -> {
            ...
            sucursalId1.getPromociones().stream().map(Promocion::getDenominacion).forEach(logger::info);
            logger.info("---------------Sucursales con la promocion id = 2---------------");
            promocionId2.getSucursales().stream().map(Sucursal::getNombre).forEach(logger::info);
            logger.info("----------------------------------------------------------------");
